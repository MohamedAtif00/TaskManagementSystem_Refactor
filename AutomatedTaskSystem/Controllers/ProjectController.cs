...
namespace AutomatedTaskSystem.Controllers
{
	...
	public class ProjectController : ControllerBase
	{
		...
		=> await _projectService.GetUnassignedUsers(id);
		...
		[Authorize]
		[HttpGet("assignment")]
		public async Task<ActionResult<ResponseService<List<Responses.ProjectDTO>>>> GetAssignedProject()
		=> await _projectService.GetUserSpecificProjects();
		// Assign to project
		[HttpPost("{id}/assign")]
		...
		// Get Project details
        [HttpGet("{id}/details")]
        public async Task<ActionResult<ResponseService<Responses.DetailedProjectDTO>>> GetProjectDetail(int id)
        => await _projectService.GetProjectDetails(id);
		// Get Project
        [HttpGet("{id}")]
		...
		// Get all Projects LO
		[HttpGet("{id}/los")]
		public async Task<ActionResult<ResponseService<List<Responses.IDName>>>> GetProjectsLo(int id)
		=> await _projectService.GetProjectLearningObjectives(id);
		// Create Project
		[HttpPost]
		public async Task<ActionResult<ResponseService<Responses.ProjectDTO>>> CreateProject(Requests.ProjectDTO req)
		=> await _projectService.CreateProject(req.Name, req.Description);
		// Add unit to project
		[HttpPost("{id}/units")]
		public async Task<ActionResult<ResponseService<Responses.ProjectUnitDTO>>> AddUnit(int id, Requests.NameDTO req)
