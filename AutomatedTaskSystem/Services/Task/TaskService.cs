...
public class TaskService : ITaskService
{
	...
	public async Task<ActionResult<ResponseService<List<GetTaskCardDto>>>> GetProjectTask(int pid)
	{
        ...
        var isProjectWide =
            user.Role == UserRoleEnum.ProjectManger || user.Role == UserRoleEnum.Owner;

        var projectInfo = await _context.Subjects
            .AsNoTracking()
            .Where(p => p.Id == pid && !p.Archived)
            .Select(p => new { IsMember = p.Users.Any(u => u.Id == user.Id) })
            .FirstOrDefaultAsync();
        ...
        var query = _context.Tasks
            .AsNoTracking()
            .Where(t => !t.Archived && t.LearningObjective.Lesson.Unit.SubjectId == pid);
        ...
        else
        {
            ...
            if (!await _context.Groups.AnyAsync(g => g.Id == user.GroupId))
                return new BadRequestObjectResult(
        ...
        // The work time total and the activity timestamps are aggregated in the same
        // statement, so the endpoint costs a fixed number of round trips regardless of
        // how many tasks the project holds.
            ...
            .Select(
                t =>
                    new
                    {
                        ...
                        t.CreatedAt,
                        t.Duration,
                        FromName = t.From == null ? null : t.From.Name,
                        ...
                        UserName = t.User == null ? null : t.User.Name,
                        DurationMs = _context.TaskWorkTimes
                            .Where(q => q.TaskId == t.Id)
                            .Sum(q => q.Duration),
                        StartedAt = _context.TaskActivities
                            .Where(
                                a =>
                                    a.TaskId == t.Id
                                    && a.Type == TaskActivityTypeEnum.Status_Doing
                            )
                            .Min(a => (DateTime?)a.TimeStamp),
                        DoneAt = _context.TaskActivities
                            .Where(
                                a =>
                                    a.TaskId == t.Id
            ...
            Data = rows.Select(
                    r =>
                        new GetTaskCardDto
                        {
                            ...
                            LearningObjective = new BasicInfoDto
                            {
                                Id = r.LearningObjectiveId,
                                Name = r.LearningObjectiveName
                            },
                            Name = r.Name,
                            Priority = r.Priority,
                            RollbackCount = r.RollbackCount,
                            Status = r.Status,
                            User = r.UserId is null
                                ? null
                                : new BasicInfoDto
                                {
                                    Name = r.UserName ?? "",
                            ...
                            duration = (decimal?)r.DurationMs / 60000,
    ...
    private async System.Threading.Tasks.Task UpdateLearningObjectiveCompletionAsync(int learningObjectiveId)
    {
        ...
        var allTerminalBranchesComplete = await AreAllTerminalBranchesCompleteAsync(lo.SchemaId, loTasks);

        if (hasActiveTasks && !hasIncompleteTasks && allTerminalBranchesComplete)
            lo.DoneAt ??= DateTime.Now;
        else
            lo.DoneAt = null;
    }

    private async Task<bool> AreAllTerminalBranchesCompleteAsync(int schemaId, List<Models.Task> loTasks)
    {
        var schemaNodes = await _context.Nodes
            .Where(n => n.SchemaId == schemaId && !n.Archived)
            .Include(n => n.Next)
            .Include(n => n.Steps)
            .ToListAsync();

        var terminalNodes = schemaNodes
            .Where(n => n.isEnd || n.Next.All(next => next.Archived))
            .ToList();
        ...
        {
            var lastStep = terminalNode.Steps
                .Where(s => !s.Archived)
                .OrderByDescending(s => s.Order)
                .FirstOrDefault();
            ...
            var lastStepTasks = loTasks.Where(t => t.StepId == lastStep.Id).ToList();
            if (lastStepTasks.Count == 0 || lastStepTasks.Any(t => t.Status != TaskStatusEnum.Done))
                return false;
        }
