...
public sealed class SubjectOverviewQueries(ISqlConnectionFactory connectionFactory)
{
    private const string AggregateSql = """
        ;WITH [ScopeLos] AS (
            SELECT lo.[Id] AS [LearningObjectiveId]
    ...
    public async Task<Result<OverviewResult>> GetBySubjectIdAsync(
        int subjectId,
        CancellationToken cancellationToken = default)
    {
        ...
        if (!exists)
        {
            return Result.Fail<OverviewResult>(AnalyticsErrors.SubjectNotFound);
        }
        ...
        return Result.Ok(MapResult(subjectId, "subject", row));
    }
