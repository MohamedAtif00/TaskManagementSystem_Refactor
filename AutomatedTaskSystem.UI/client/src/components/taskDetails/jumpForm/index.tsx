...
const JumpForm: React.FC<Props> = ({ taskId, updateTask, type }) => {
    ...
    return (
        <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed z-50 top-0 bottom-0 left-0 right-0 backdrop-blur-sm bg-black/25 flex justify-center items-center"
        >
            {points === undefined ? (
                <Loader />
            ) : (
                    ...
                    <div className="px-6 py-4">
                        {loadError ? (
                            ...
                            <div className="text-sm text-slate-600 py-2">
                                No jump destinations are available for this
                                task.
                            </div>
                        ) : (
                            <>
