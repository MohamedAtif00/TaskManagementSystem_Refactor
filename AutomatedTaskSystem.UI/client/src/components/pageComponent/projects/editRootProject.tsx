...
const EditRootProject = () => {
    ...
    useEffect(() => {
        if (query.form === "edit-root" && !Number.isNaN(rootId)) {
            API.PROJECTS.ROOT.GET(rootId).then((res) => {
                if (res && typeof res === "object" && "error" in res && !res.error && "data" in res) {
                    const d = (res as { data: { name: string; description: string } }).data;
                    setName(d.name);
    ...
    const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        API.PROJECTS.ROOT.UPDATE(rootId, name.trim(), description).then((res) => {
...
export default EditRootProject;
