...
const EditGroup = () => {
    ...
    useEffect(() => {
        if (query.form === "edit-group" && query.groupId) {
            API.RESOURCES.GROUPS.GET_ONE(query.groupId).then((res) => {
                if (res && !res.error) {
                    setName(res.data.name);
                    setColor(() => {
                        const foundColor = codes.find(
                            (c) => c.name === res.data.colorCode
                        );

                        if (foundColor) return foundColor;

                        return { id: 0, name: res.data.colorCode };
                    });
                }
            });
