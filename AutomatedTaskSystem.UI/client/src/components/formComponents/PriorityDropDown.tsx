...
const PriorityDropDown = ({ value, handleChange }: Props) => {
			...
			{active && (
				...
				>
					<div
						onClick={() => {
							setActive(false);
							handleChange({ id: 1, name: "High" });
						}}
						className="px-4 basis-8 shrink-0 hover:bg-slate-300 hover:text-black text-slate-600 flex items-center gap-2"
					>
						<div className="p-2 rounded-full bg-red-600"></div>
						<div className="capitalize">High</div>
					</div>
					<div
						onClick={() => {
							setActive(false);
							handleChange({ id: 2, name: "Medium" });
						}}
						className="px-4 basis-8 shrink-0 hover:bg-slate-300 hover:text-black text-slate-600 flex items-center gap-2"
					>
					...
					<div
						onClick={() => {
							setActive(false);
							handleChange({ id: 3, name: "Low" });
						}}
						className="px-4 basis-8 shrink-0 hover:bg-slate-300 hover:text-black text-slate-600 flex items-center gap-2"
					>
						<div className="p-2 rounded-full bg-blue-600"></div>
						<div className="capitalize">Low</div>
					</div>
					<div
						onClick={() => {
							setActive(false);
							handleChange({ id: 4, name: "None" });
						}}
						className="px-4 basis-8 shrink-0 hover:bg-slate-300 hover:text-black text-slate-600 flex items-center gap-2"
					>
						<div className="p-2 rounded-full bg-white shadow-sm"></div>
