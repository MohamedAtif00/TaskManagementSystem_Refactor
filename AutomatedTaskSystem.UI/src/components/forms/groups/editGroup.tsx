...
import { edit } from "../../../slices/groupSlice";
...
const EditGroup = ({ groups }: { groups: IGroup[] }) => {
	...
	useEffect(() => {
		const _active =
			router.query.form === "editGroup" && router.query.groupId != undefined;
		setActive(_active);
		if (!_active) {
			setName("");
		}
	...
	useEffect(() => {
		if (router.query.groupId) {
			const foundGroup = groups.find(
				(g) => g.id == parseInt(router.query.groupId!.toString())
			);
			if (foundGroup) {
				setName(foundGroup.name);
				setColor(colorList.find((c) => c.color == foundGroup.colorCode)!.id);
			}
		}
